...
$tousLesArticles = Article::getAll($conn);

$parPage = 3;
$totalArticles = count($tousLesArticles);
$nombreDePages = ceil($totalArticles / $parPage);
...
$articles = array_slice($tousLesArticles, $indexDepart, $parPage);
?>
            ...
            <div class="flex justify-between h-20 items-center">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <?php else: ?>
                        ...
                        <a href="register.php"
                            class="px-5 py-2.5 bg-dark text-white text-sm font-semibold rounded-lg hover:bg-gray-800 transition shadow-lg">Inscription</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
    ...
    <div class="pt-32 pb-12 bg-white border-b border-gray-100">
        <div class="max-w-7xl mx-auto px-6 lg:px-8 text-center">
            ...
            <div class="mt-8 flex flex-col md:flex-row justify-between items-center gap-4 max-w-4xl mx-auto">
                <div class="relative w-full md:w-2/3">
                    ...
                    <input type="text" placeholder="Rechercher un article..."
                        class="w-full bg-gray-50 border border-gray-200 text-dark pl-11 pr-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition">
                </div>

                <?php if (isset($_SESSION['user'])): ?>
                    <a href="create_article.php"
                        class="w-full md:w-auto px-6 py-3 bg-primary hover:bg-primary_hover text-white font-bold rounded-xl transition shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2 whitespace-nowrap">
            ...
            <div class="mt-6 flex flex-wrap justify-center gap-2">
                <a href="#"
                    class="px-4 py-1.5 rounded-full bg-blue-50 text-primary text-sm font-semibold hover:bg-blue-100 transition">#Roadtrip</a>
                <a href="#"
                    class="px-4 py-1.5 rounded-full bg-gray-100 text-gray_text text-sm font-semibold hover:bg-gray-200 transition">#Mécanique</a>
                <a href="#"
                    class="px-4 py-1.5 rounded-full bg-gray-100 text-gray_text text-sm font-semibold hover:bg-gray-200 transition">#Tests</a>
            </div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <?php if (empty($articles)): ?>
                <div class="col-span-full text-center py-10">
                    <i class="fa-regular fa-newspaper text-4xl text-gray-300 mb-4"></i>
                ...
                <?php foreach ($articles as $article): ?>
                    <article
                        ...
                        <div class="relative h-56 overflow-hidden bg-gray-100">
                            <?php $img = !empty($article['image_url']) ? htmlspecialchars($article['image_url']) : 'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=800&q=80'; ?>

                            <img src="<?= $img ?>" alt="<?= htmlspecialchars($article['titre']) ?>"
                                class="w-full h-full object-cover group-hover:scale-105 transition duration-500">

                            <span
                                class="absolute top-4 left-4 bg-white/90 backdrop-blur text-primary text-xs font-bold px-3 py-1.5 rounded-md shadow-sm uppercase">
                        ...
                        <div class="p-6 flex flex-col flex-grow">
                            <div class="flex items-center gap-2 text-xs text-gray-400 mb-3">
                                <span><i class="fa-regular fa-calendar"></i>
                                    <?= date('d M Y', strtotime($article['date_creation'])) ?></span>
                                <span>•</span>
                                <span><i class="fa-regular fa-user"></i>
                                    <?= htmlspecialchars($article['auteur'] ?? 'Anonyme') ?></span>
                            </div>
                            ...
                            <div class="mt-auto pt-4 border-t border-gray-50 flex justify-between items-center">
                                <div class="flex items-center gap-4 text-gray-400 text-sm">
                                    <span class="hover:text-red-500 cursor-pointer transition"><i
                                            class="fa-regular fa-heart"></i> 0</span>
                                    <span class="hover:text-primary cursor-pointer transition"><i
                                            class="fa-regular fa-comment"></i> 0</span>
                                </div>
                                <a href="article.php?id=<?= $article['id'] ?>"
                                    class="text-primary font-bold text-sm hover:underline">Lire la suite &rarr;</a>
                            </div>
                        </div>
                    </article>
                <?php endforeach; ?>
            ...
            <nav class="inline-flex rounded-lg shadow-sm border border-gray-200 bg-white">
                ...
                <?php for ($i = 1; $i <= $nombreDePages; $i++): ?>
                    <?php if ($i == $pageActuelle): ?>
                        <span class="px-4 py-2 bg-primary text-white border-r border-primary"><?= $i ?></span>
                    <?php else: ?>
                        <a href="?page=<?= $i ?>"
                            class="px-4 py-2 text-gray-700 hover:text-primary hover:bg-gray-50 border-r border-gray-200 transition"><?= $i ?></a>
                    <?php endif; ?>
                <?php endfor; ?>
